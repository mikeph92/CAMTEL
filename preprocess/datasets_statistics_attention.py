...
def write_statistics(args, img_bg_fg, cluster, statistics):
    ...
    df.to_excel(f'./statistics/{args.classification_task}_{args.testset}/{img_bg_fg}_cluster_{cluster}.xlsx', index=False)

    l_avg_mean = np.mean(statistics[0]).item()
    l_avg_std = np.std(statistics[0]).item()
    l_std_mean = np.mean(statistics[1]).item()
    l_std_std = np.std(statistics[1]).item()
    a_avg_mean = np.mean(statistics[2]).item()
    a_avg_std = np.std(statistics[2]).item()
    a_std_mean = np.mean(statistics[3]).item()
    a_std_std = np.std(statistics[3]).item()
    b_avg_mean = np.mean(statistics[4]).item()
    b_avg_std = np.std(statistics[4]).item()
    b_std_mean = np.mean(statistics[5]).item()
    b_std_std = np.std(statistics[5]).item()

    # std_avg_list = [labL_avg_List, labL_std_List, labA_avg_List, labA_std_List, labB_avg_List, labB_std_List]
    distribution = []
    for std_avg in statistics:
    ...
    yaml_save_path = '{}/{}_{}.yaml'.format(save_path, args.color_space, cluster)
...
if __name__ == '__main__':
    ...
    for img_bg_fg in img_bg_fg_list:
        ...
        for _,row in df.iterrows():
            ...
            # save_dir = os.path.dirname(origin_path).replace('/nfs/datasets/public/ProcessedHistology/', '/nfs/users/michael/histoPath/')
        ...
        t2 = time.time()
        print(t2 - t1)
        save_path = f'./statistics/{args.classification_task}_{args.testset}/{img_bg_fg}'
        if not os.path.exists(save_path):
            os.makedirs(save_path)
        
        #write statistics for full set
        statistics = [labL_avg_List, labL_std_List, labA_avg_List, labA_std_List, labB_avg_List, labB_std_List]
        write_statistics(args, img_bg_fg, "full", statistics)

        #write statistics for each cluster
        for i in df["labelCluster"].unique():
            indices = df.index[df['labelCluster'] == i]
            statistics_filtered = [[arr[j] for j in indices] for arr in statistics]
            write_statistics(args, img_bg_fg, i, statistics)
