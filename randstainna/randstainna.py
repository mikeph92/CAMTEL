...
class RandStainNA(object):
    ...
    def augment(self, img): 
        ...
        image = cv2.cvtColor(image_np, cv2.COLOR_RGB2LAB)
        num_of_channel = image.shape[2]
...
if __name__ == '__main__':
    ...
    df =  pd.read_csv(f"../clustering/output/clustering_result_{task}_{testset}.csv")
    clusters = df.labelCluster.unique().tolist()
    clusters.append("all")

    for cluster in clusters:
        cluster_text = f"_{cluster}" if cluster != "all" else ""

        yaml_path = f'../yaml_config/{task}_{testset}{cluster_text}.yaml'
        ...
        save_dir_path = f'/nfs/users/michael/randstainna/{task}_{testset}/{cluster_text}'
        if not os.path.exists(save_dir_path):
            os.makedirs(save_dir_path, exist_ok=True)

        for img_path in img_path_list:
            img = randstainna(img = Image.open(img_path).convert('RGB'))
